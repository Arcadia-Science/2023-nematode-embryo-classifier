...
The script requires only the `--data-dirpath` option; it automatically loads the features generated in step (4) from the `encoded_dynamics` subdirectory for the subset of embryos (from any number of FOVs and datasets) that were manually annotated.

The model weights and logs are exported in `models` folder, which looks like this:
```
models
└── lightning_logs
   └── sulstonNet_heatshock_7classes_raw
      ├── checkpoints
